...
class MNIST_embedding_visualization():
    ...
    def intra_distances(self, landmarks):
    ...
    def voltageSpace_convexHull(self, max_dist, voltage_embedding, landmark_indices):
        distances = cdist(voltage_embedding, voltage_embedding)
        dist_to_landmarks = distances[:, landmark_indices]

        bool = dist_to_landmarks > max_dist
        indices_inside_hull = np.sum(bool, axis=1) == 0
    ...
    def visualize_voltageSpace_convexHull(self, level, digits, landmark_numbers):
        ...
        max_dist = self.intra_distances(lm_v_emb)
        ...
        self.plot_images_xy_plane(mds_emb, landmark_indices, source_indices, level, digits, nlm, experiment_name='convexhull_vs')
    ...
    def plot_images_xy_plane(self, mds_emb, idx_lms, idx_sources, level, digits, nlm, experiment_name):
        cmap = color_map()
        ...
        indices = np.random.choice(np.arange(0, len(self.data)), size=np.min([1000, len(self.data)]), replace=False)
        for idx in indices:
            digit = int(self.labels[idx])
            mnist_lm = self.data[idx]
        ...
        for idx in idx_lms:
            ...
            mnist_lm = self.data[idx]
            mnist_lm = mnist_lm.reshape(28, 28)
            imagebox = OffsetImage(mnist_lm, zoom=1.5, cmap=cmap_mnist[digit+2])
            xy = [mds_emb[idx, 0], mds_emb[idx, 1]]  # coordinates to position this image
